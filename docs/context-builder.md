...
- 代码：`src/context_builder/`（模块本体 + 平台侧两个 resolver）、`src/context_resolver.go`（runtime 自己的 resolver、接线、挂点）。
...
两个注册表都**读得很省**：成功缓存 30s、失败缓存 5s（按 resolver 实例），一次 run 的多个 cycle 基本只读一次。
...
4. **顺序即优先级**：注册顺序决定字段归属，加一个新来源（例如将来的 `service-registry`：项目 → 服务）就是加一个 resolver。
