...
- 之前的每轮结果都作为 `DecisionContext.History` 传给下一次 decide，prompt 的 Runtime Context 里渲染成 `previous_actions: [{step,message,status,error,output}]`（正是 policy 里 `previous_action` 证据来源）。其中 `output` 就是该轮 action 的 capability 输出（例如 `code_edit` 的 `summary`/`workspace`），所以 planner 能看到"上一轮到底做成了什么"再决定是否 done。
