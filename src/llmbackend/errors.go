...
// BridgeCallErr is bridgeCallErr for callers outside this package (the runtime's own tests
// read the same reading): a failed bridge call, read against the context it ran under.
func BridgeCallErr(ctx context.Context, err error) error { return bridgeCallErr(ctx, err) }

// EmptyModelResponseErr is the failure a run that produced no text at all is.
func EmptyModelResponseErr(status, msg string) error { return emptyModelResponseErr(status, msg) }
